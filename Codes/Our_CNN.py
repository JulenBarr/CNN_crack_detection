...
for i in range(1):
  ...
  removed_id = np.array([84, 85, 93, 97, 101, 102, 104, 108, 115])-1
...
n_train = 72 # number of training samples 30
n_test = 35 # number of testing samples
...
print('Training samples:')
precision, recal, f1, prob_maps = ut.crack_pred(cnn, label_path, image_path, fig_path, train_id[:72], h=h, s=s, prob=0.6) #prob 0.5 default
...
print("Average Training F1:        {:0.8f}".format(f1))
  ...
  # os.makedirs(fig_path, exist_ok=True)
print('Testing samples:')
precision_test, recall_test, f1_test, prob = ut.crack_pred(cnn, label_path, image_path, fig_path, test_id[:35], h=h, s=s, prob=0.5)
